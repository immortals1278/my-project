...
contract UniswapV2Router{
    ...
    function _calculateLiquidity(
        ...
        (uint256 reserveA,uint256 reserveB) = IUniswapV2Library.getReserves(
            address(factory),
            tokenA,
            tokenB
        );
    ...
    function swapExactTokensForTokens(
        ...
        _safeTransferFrom(
            path[0],
            msg.sender,
            IUniswapV2Library.getPair(address(factory),path[0],path[1]),
            amounts[0]
        );
    ...
    function _swap(
        ...
        for(uint256 i;i < path.length - 1;i++){
            ...
            uint256 amountsOut = amounts[i + 1];
            (uint256 amount0Out,uint256 amount1Out) = input == token0 ?
            (uint256(0),amountsOut)
            :(amountsOut,uint256(0));
            to = i < path.length - 2 
            ? IUniswapV2Library.getPair(address(factory),output,path[i + 2])
            : to_;
            IUniswapV2Pair(
                IUniswapV2Library.getPair(address(factory),input,output)
